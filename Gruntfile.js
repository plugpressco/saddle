...
module.exports = function ( grunt ) {
	...
	var MAIN_FILE = 'saddle.php';
	var SLUG      = 'saddle';

	function currentVersion() {
		var php   = grunt.file.read( MAIN_FILE );
		var match = php.match( /Version:\s*([0-9]+\.[0-9]+\.[0-9]+)/ );
		if ( ! match ) {
	...
	grunt.initConfig( {
			...
			dist: {
				files: [ {
					...
					src: [
						...
						// Ship the compiled admin bundle, not its React source.
						'!admin/src/**',
						// OS / editor cruft.
						'!**/.DS_Store',
						'!**/Thumbs.db',
		...
		compress: {
			dist: {
				...
				options: { mode: 'zip' },
				files: [ { expand: true, cwd: 'dist/', src: [ SLUG + '/**' ], dest: '' } ],
			},
		},
	} );
	...
	grunt.registerTask( 'version', 'Bump the plugin version (patch|minor|major, or --to=x.y.z).', function ( type ) {
		var from = currentVersion();
		var to   = grunt.option( 'to' ) || bump( from, type || 'patch' );
		...
		php = php.replace( /(define\(\s*'SADDLE_VERSION',\s*')[0-9.]+('\s*\))/, '$1' + to + '$2' );
	...
	grunt.registerTask( 'zip', 'Name the archive after the current version.', function () {
		var archive = 'dist/' + SLUG + '-' + currentVersion() + '.zip';
		grunt.config.set( 'compress.dist.options.archive', archive );
		grunt.log.ok( 'Archive: ' + archive );
	} );

	grunt.registerTask( 'build', [ 'clean:dist', 'copy:dist', 'zip', 'compress:dist' ] );
