...
3. **WP-CLI-equivalent native abilities**, ~12-15 ops, PHP-native dispatch (no shell, ever — see scope lock): plugin list/activate/deactivate, theme list/activate, option get/update (allowlisted keys only, block siteurl/auth keys/salts same as the competitor pattern already established), cache flush. This was Tier-1 in the original 80/20 analysis and got cut from v0.1 purely on scope-narrowing instruction, not because it's low value — revisit first in Phase 2.
