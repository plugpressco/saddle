...
- **Free/pro line (decided 2026-07-03):** free = core content + safety + **native WordPress design** (structured Gutenberg block editing: the generic tree engine with a Gutenberg validation profile, block schemas from `WP_Block_Type_Registry`, theme.json design tokens, block patterns). Pro = **page-builder-native depth** (Divi first). **Safety is never paywalled.** The upsell: "Saddle designs with WordPress's native editor; Saddle Pro does it with your page builder." Architectural consequence: the builder-agnostic tree core (parse/address/mutate/serialize + pluggable validation profiles) lives in FREE — Pro's Divi layer ships only the Divi profile, detection, schemas, and playbook, and consumes the free engine (Pro already hard-requires free).

## Sequencing

1. **Saddle Pro Divi MVP** (this doc) — jumped ahead of everything per 2026-07-02 priority call: Divi first, because distribution. While building its write abilities, extract the generic tree core INTO FREE (Pro consumes it) — that's the enabler for #2.
2. **Free: Gutenberg design abilities** (decided 2026-07-03) — the tree engine's Gutenberg profile + block-tree tools, list/get-block-schema from the registry, theme.json tokens, list/insert block patterns. Fast follow after the Divi MVP; mostly reuse.
3. Free v0.2 site-management abilities (plugins/themes/options/cache + third "Full management" UI level) — fully mapped, resumes after the above or interleaved.
4. Pro v0.2 candidates, evidence-driven: Divi Theme Builder + presets/global styles (gated, site-scoped), Divi Library, loops/dynamic content; or a second builder if buyers ask.
