...
4. **Free: Gutenberg design abilities** ✅ DONE (2026-07-03) — the tree engine's Gutenberg profile (`Saddle_Blocks_Tree`) + authoring layer (`Saddle_Blocks_Author`: content/attrs → editor-valid markup with preset classes) + 11 abilities: get/set-blocks, add/edit/move/remove-block (remove gated on subtrees), insert-block-pattern, list-block-types, get-block-schema, get-design-tokens, list-block-patterns. Builder pages refused (routed to divi-* tools when Pro is present). PHPUnit-covered; live round-trip pending.
